...
async def _(event):
    ...
    if userid == 1118936839:
        ...
        packshortname = f"Zayn_pack_tony"
    await event.edit(
    ...
    if is_a_s:
        ...
        packname = f"{user.first_name}'s Animated"
        if userid == 1118936839:
            packshortname = f"Survivor_Animated"
        else:
            packshortname = (
                f"{user.username}_animate_{user.id}"  # format: Uni_Borg_userid
            )
